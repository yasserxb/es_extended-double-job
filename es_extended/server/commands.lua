...
ESX.RegisterCommand('setfaction', 'admin', function(xPlayer, args, showError)
    if not ESX.DoesFactionExist(args.faction, args.grade) then
        return showError("Commande setFaction Invalide")
    end
...
ESX.RegisterCommand("job2", { "user", "admin" }, function(xPlayer, _, _)
    print(("%s, your job2 is: ^5%s^0 - ^5%s^0"):format(xPlayer.getName(), xPlayer.getFaction().name, xPlayer.getFaction().grade_label))
end, false)

ESX.RegisterCommand("info", { "user", "admin" }, function(xPlayer)
    local job = xPlayer.getJob().name
    local faction = xPlayer.getFaction().name
    print(("^2ID: ^5%s^0 | ^2Name: ^5%s^0 | ^2Group: ^5%s^0 | ^2Job: ^5%s^0| ^2Faction: ^5%s^0"):format(xPlayer.source, xPlayer.getName(), xPlayer.getGroup(), job,faction))
end, false)
