...
local loadPlayer =
"SELECT `accounts`, `job`, `job_grade`, `faction`, `faction_grade`, `group`, `position`, `inventory`, `skin`, `loadout`, `metadata`"
...
function loadESXPlayer(identifier, playerId, isNew)
    ...
    if ESX.DoesFactionExist(faction, gradef) then
        ...
        print(('[^3WARNING^7] Ignoring invalid faction for %s [faction: %s, grade: %s]'):format(identifier, faction,
            gradef))
    ...
    local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory,
        userData.weight, userData.job, userData.faction, userData.loadout, GetPlayerName(playerId), userData.coords, userData.metadata)
